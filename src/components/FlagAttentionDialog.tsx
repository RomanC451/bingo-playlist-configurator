...
interface FlagAttentionDialogProps {
  open: boolean;
  loading?: boolean;
  trackName?: string;
  onClose: () => void;
...
export function FlagAttentionDialog({
  open,
  loading = false,
  trackName,
  onClose,
  onSubmit,
}: FlagAttentionDialogProps) {
  const titleId = useId();
    ...
    >
      ...
      >
        ...
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
          {trackName
            ? `What needs to be fixed on “${trackName}”?`
            : "What needs to be fixed on this track?"}
        </p>
